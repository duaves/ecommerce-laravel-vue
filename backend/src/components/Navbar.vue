<template>
    <header class="flex justify-between items-center p-3 h-14 shadow bg-white">
        <button @click="emit('toggle-sidebar')"
            class="flex items-center justify-center rounded transition-colors w-8 h-8 text-gray-500 hover:bg-black/10">
            <Bars3Icon class="w-6" />
        </button>
        <Menu as="div" class="relative inline-block text-left">
            <MenuButton class="flex items-center">
                <img src="https://randomuser.me/api/portraits/lego/4.jpg" class="rounded-full w-8 mr-2">
                ...
                <MenuItems
                    ...
                    <div class="px-1 py-1">
                        ...
                        <button @click="logout" :class="[
                            ...
                            <ArrowRightOnRectangleIcon :active="active" class="mr-2 h-5 w-5 text-indigo-400"
                                aria-hidden="true" />
                            Выйти
                        </button>
                        </MenuItem>
...
<script setup>
import { UserIcon, ArrowRightOnRectangleIcon, Bars3Icon } from '@heroicons/vue/24/outline'
